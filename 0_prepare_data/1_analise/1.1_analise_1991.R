...
# 
# cp1991_nc <- write_dta(censo_1991_raw, "E:/Thais/Tese_Mestrado/censo_1991/censo_1991_raw.dta")
# cp1991_nc <- read_dta("E:/Thais/Tese_Mestrado/censo_1991/censo_pessoas_1991_sem_comp.dta")
# write_parquet(cp1991_nc, "E:/Thais/Tese_Mestrado/censo_1991/censo_pessoas_1991_sem_comp.parquet")
# cp1991_nc <- read_parquet("E:/Thais/Tese_Mestrado/censo_1991/censo_pessoas_1991_sem_comp.parquet")

# cp1991_nc <- read_dta("E:/Thais/Tese_Mestrado/censo_1991/censo_pessoas_1991_filtrado_nao_comp.dta")
# 
# write_parquet(cp1991_nc, "E:/Thais/Tese_Mestrado/censo_1991/censo_pessoas_1991_filtrado_nao_comp.parquet")

cp1991_nc <- read_parquet("E:/Thais/Tese_Mestrado/censo_1991/censo_pessoas_1991_filtrado_nao_comp.parquet")
...
cp1991_nc_copy <- cp1991_nc

# Renomeando países de origem
cp1991_nc <- cp1991_nc %>% rename(pais_nascim = V0316)
cp1991_nc_copy <- cp1991_nc_copy %>% rename(pais_nascim = V0316)

setDT(cp1991_nc, cp1991_nc_copy)


cp1991_nc <- cp1991_nc_copy %>%
...
cp1991_nc <- cp1991_nc %>% 
  ...
  filter(V7003 != 0) #so quem mora em RM

cp1991_nc <- cp1991_nc %>%
  mutate(
    ano_nasc = ano - V3072,
    imm = ifelse((V3152 - ano_nasc <= 5), 1, 0),
...
imm_div_base <- cp1991_nc %>% filter(imm_div == 1)
...
cp1991_nc$imm_div[is.na(cp1991_nc$imm_div)] <- 0
...
# cp1991_nc com home country divorce rate
cp1991_nc_1 <- merge(cp1991_nc, home_country_div_rate_1990, by = "pais_nascim", all.x = TRUE)
#----
# # com gdp_pc: considerar ano 2000
# # gdp_pc_2000 <- gdp_pc %>% filter(ano_gdp_pc == 2000)
# 
# cp1991_nc_2 <- merge(cp1991_nc_1, gdp_pc_2000, by = "pais_nascim", all.x = TRUE)
# 
# # com age_f_first_marriage
# cp1991_nc_3 <- merge(cp1991_nc_2, age_ffm, by = "pais_nascim", all.x = TRUE)
# 
# # com fertility_rate
# cp1991_nc_4 <- merge(cp1991_nc_3, fertility_rate, by = "pais_nascim", all.x = TRUE)
# 
# # com fem lfp
...
cp1991_nc_1 <- cp1991_nc_1 %>% 
  filter(!`pais_nascim` %in% c("OTH_AFR", "OTH_AM", "OTH_ASIA","OTH_EUR", "OTH_OCE"))

cp1991_nc_1 <- cp1991_nc_1 %>% rename(escolaridade = V0328)
cp1991_nc_1$fund_completo <- ifelse(cp1991_nc_1$escolaridade == 3 | cp1991_nc_1$escolaridade == 4, 1, 0)
cp1991_nc_1$med_completo <- ifelse(cp1991_nc_1$escolaridade == 5 | cp1991_nc_1$escolaridade == 6, 1, 0)
cp1991_nc_1$sup_completo <- ifelse(cp1991_nc_1$escolaridade == 7, 1, 0)

# transforma escolaridade em continua
cp1991_nc_1$escolaridade_cont <- as.numeric(cp1991_nc_1$escolaridade)
...
paises_para_remover <- c("AGO", "ARG", "BEL", "BGR", "BLZ", "BOL",    
                         ...
                         "LBN", "MOZ", "NIC", "NLD", "NO_SPEC", "PAK",    
                         "PER", "PRY", "SUR", "SYR", "TWN", "YUG")

# Filtrando o DataFrame para remover os países com NA em hcdr
cp1991_nc_1 <- cp1991_nc_1[!cp1991_nc_1$pais_nascim %in% paises_para_remover, ]

# Remove na coluna pais_nascim com valores NA
cp1991_nc_1 <- cp1991_nc_1[!is.na(cp1991_nc_1$pais_nascim), ]

base_partial_nc_1991 <- as.data.frame(cp1991_nc_1)
...
# FE: idade
model1_nc <- feols(imm_div ~ hcdr + imm_homem + fund_completo + med_completo + sup_completo | V3072 + age_squared, data = base_partial_nc_1991, weights = ~V7301)
summary(model1_nc)

# FE: idade com escolaridade continua
model1_1_nc <- feols(imm_div ~ hcdr + imm_homem + escolaridade_cont | V3072 + age_squared, data = base_partial_nc_1991, weights = ~V7301)
summary(model1_1_nc)

# FE: idade e RM
model2_nc <- feols(imm_div ~ hcdr + imm_homem + escolaridade_cont | V3072+age_squared+V7003, data = base_partial_nc_1991, weights = ~V7301)
summary(model2_nc)
...
model2_nc_sem_portugal <- feols(imm_div ~ hcdr + imm_homem + escolaridade_cont | V3072+age_squared+V7003, data = base_partial_nc_sem_portugal, weights = ~V7301)
summary(model2_nc_sem_portugal)

# Drop Russia and Italy (4)
# Ensure dplyr functions are used explicitly
max_hcdr_info <- base_partial_nc_1991 %>%
  dplyr::filter(hcdr == max(hcdr, na.rm = TRUE)) %>%
  dplyr::select(pais_nascim, hcdr)

min_hcdr_info <- base_partial_nc_1991 %>%
...
model2_nc_sem_russia_italy <- feols(imm_div ~ hcdr + imm_homem + escolaridade_cont | V3072+age_squared+V7003, data = base_partial_nc_sem_russia_italy, weights = ~V7301)
